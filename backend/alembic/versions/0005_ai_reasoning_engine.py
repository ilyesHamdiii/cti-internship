...
def upgrade() -> None:
    ...
    if "ai_reasoning_sessions" not in tables:
        op.create_table(
    ...
    if "ai_reasoning_revisions" not in tables:
        op.create_table(
    ...
    if "ai_watcher_results" not in tables:
        op.create_table(
            "ai_watcher_results",
            sa.Column("id", postgresql.UUID(as_uuid=False), primary_key=True),
            sa.Column("session_id", postgresql.UUID(as_uuid=False), sa.ForeignKey("ai_reasoning_sessions.id"), nullable=False),
    ...
    if "ai_confidence_events" not in tables:
        op.create_table(
    ...
    if "cti_consumption_records" not in tables:
        op.create_table(
    ...
    if "ix_ai_reasoning_revisions_session_created" not in existing_indexes:
        op.create_index(
            "ix_ai_reasoning_revisions_session_created",
            "ai_reasoning_revisions",
            ["session_id", "created_at"],
        )
    if "ix_ai_watcher_results_session_created" not in existing_indexes:
        op.create_index(
            "ix_ai_watcher_results_session_created",
            "ai_watcher_results",
            ["session_id", "created_at"],
        )
    if "ix_ai_confidence_events_session_created" not in existing_indexes:
        op.create_index(
            "ix_ai_confidence_events_session_created",
            "ai_confidence_events",
            ["session_id", "created_at"],
        )
    if "ix_cti_consumption_records_created" not in existing_indexes:
        op.create_index(
            "ix_cti_consumption_records_created", "cti_consumption_records", ["created_at"]
        )
