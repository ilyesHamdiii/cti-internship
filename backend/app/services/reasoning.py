...
class WatcherEngine:
    def evaluate_behavior_extraction(
        self, normalized_cti: dict[str, Any], response: dict[str, Any]
    ) -> list[dict[str, Any]]:
        behaviors = response.get("behaviors", []) if isinstance(response, dict) else []
        attributes = (
            normalized_cti.get("attributes", []) if isinstance(normalized_cti, dict) else []
        )
        evidence_refs = {
        ...
        return [
            self._schema(
                "schema",
                bool(behaviors),
                "Behavior extraction returned structured behavior objects",
            ),
            self._schema(
                "evidence",
                all(
                    ref.get("ref") in evidence_refs
                    for behavior in behaviors
                    for ref in behavior.get("evidence_refs", [])
                ),
            ...
            self._schema(
                ...
                all(
                    any(
                        str(attr.get("value", "")) in str(behavior)
            ...
            ),
            {
                "watcher_name": "safety",
                "status": PASS,
                "message": "No executable action was taken by the AI provider",
                "details": {"scope": "structured_output_only"},
            },
        ]
    ...
    def evaluate_candidate(
        ...
        return [
            self._schema(
                "schema",
                bool(candidate.get("title") and candidate.get("detection")),
                "Sigma candidate contains required structured fields",
            ),
            self._schema(
                "sigma",
                bool(validation.get("valid")),
                "pySigma validation and compilation completed successfully",
            ),
            self._schema(
                "confidence",
                ai_confidence >= 0.45,
                "AI confidence is above the minimum usable threshold",
                warning=True,
            ),
            self._schema(
                "duplicate",
                duplicate_status != "exact_duplicate",
                f"Duplicate status is {duplicate_status or 'unknown'}",
            ),
            self._schema(
                "telemetry",
                bool(telemetry_fields),
                "Required telemetry fields are available to validate detection feasibility",
                warning=True,
            ),
            self._schema(
                "hallucination",
                bool(candidate.get("detection")),
                "Detection logic is present and bounded to structured Sigma fields",
            ),
            self._schema(
                "safety",
                not (safety_text.count("delete ") > 2 or "supervisor_safety_fail" in safety_text),
                "Candidate does not contain unsafe operational instructions",
                warning=False,
            ),
        ]

    def _schema(
        self, name: str, condition: bool, message: str, warning: bool = False
    ) -> dict[str, Any]:
        if condition:
            return {
                "watcher_name": name,
...
class ConfidenceEngine:
    def assess(
        ...
        validation_score = (
            1.0 if validation.get("valid") else 0.25 if validation.get("repairable") else 0.0
        )
        watcher_score = self._watcher_score(watchers)
        telemetry_score = (
            1.0 if visibility_status == "visible" else 0.55 if visibility_status else 0.35
        )
        coverage_score = (
            0.85
...
class TrustEngine:
    def assess(
        self, validation: dict[str, Any], watchers: list[dict[str, Any]], confidence: float
    ) -> dict[str, Any]:
        ...
        deterministic = (
            1.0 if validation.get("valid") else 0.2 if validation.get("repairable") else 0.0
        )
        trust = max(
            0.0,
            min(
...
class SatisfactionEngine:
    mandatory_watchers: ClassVar[set[str]] = {"schema", "safety", "sigma", "telemetry"}
    ...
    def assess(
        ...
        missing_mandatory = [
            name for name in self.mandatory_watchers if by_name.get(name, {}).get("status") != PASS
        ]
        if by_name.get("safety", {}).get("status") == FAIL:
            return self._result(
                False, "blocking_fail", "reject", "safety", "safety_watcher_failed", False, warnings
            )
        ...
        if by_name.get("telemetry", {}).get("status") in {FAIL, WARNING}:
            return self._result(
                False,
                "repairable_fail" if validation.get("repairable") else "blocking_fail",
                "terminal_visibility_gap",
                "telemetry",
                "telemetry_watcher_failed",
                bool(validation.get("repairable")),
                warnings,
            )
        ...
        if confidence < settings.reasoning_confidence_target:
            return self._result(
                False,
                "repairable_fail",
                "repair_candidate" if validation.get("repairable") else "queue_review",
                "confidence",
        ...
        if trust < settings.reasoning_trust_target:
            return self._result(
                False, "warning", "queue_review", "trust", "trust_below_target", False, warnings
            )
        if failing:
            return self._result(
                False,
                "repairable_fail",
                "repair_candidate" if validation.get("repairable") else "terminal_failed",
                str(failing[0].get("watcher_name")),
                "watcher_failed",
                bool(validation.get("repairable")),
                warnings,
            )
    ...
    def _result(
        ...
        return {
            ...
            if route == "queue_review"
            else route.replace("terminal_", ""),
        }
...
class ReasoningService:
    ...
    def start_session(
        self, workflow: Workflow, graph_run: GraphRun, resume_from_node: str | None = None
    ) -> AiReasoningSession:
        if resume_from_node:
            ...
            if existing_for_workflow:
                ...
                existing_for_workflow.updated_at = datetime.now(UTC)
                self.db.commit()
                return existing_for_workflow
        existing = self.db.scalar(
            select(AiReasoningSession).where(AiReasoningSession.graph_run_id == graph_run.id)
        )
        if existing:
            return existing
    ...
    def session_for_run(self, graph_run_id: str) -> AiReasoningSession | None:
        direct = self.db.scalar(
            select(AiReasoningSession).where(AiReasoningSession.graph_run_id == graph_run_id)
        )
        if direct:
            return direct
        ...
        return self.db.scalars(
            select(AiReasoningSession)
            .where(AiReasoningSession.workflow_id == graph_run.workflow_id)
            .order_by(AiReasoningSession.started_at.desc())
        ).first()
    ...
    def record_watchers(
        self, graph_run_id: str, node_name: str, watchers: list[dict[str, Any]]
    ) -> None:
        session = self.session_for_run(graph_run_id)
        if session is None:
            return
        ...
        failures.extend(
            jsonable_encoder(watcher) for watcher in watchers if watcher.get("status") == FAIL
        )
    ...
    def record_confidence(
        ...
        summary = dict(session.session_summary or {})
        summary.setdefault("confidence_progression", []).append(
            {
                "node": node_name,
                "score": score,
    ...
    def record_revision(
        ...
        parent = self.db.scalars(
            ...
            .order_by(AiReasoningRevision.revision_number.desc())
        ).first()
        ...
        self.db.add(
            AiReasoningRevision(
                ...
                validation_delta={
                    "confidence_delta": delta,
                    "valid_after": bool((validation_after or {}).get("valid")),
                },
        ...
        summary.setdefault("repair_history", []).append(
            {
                "stage": stage,
                "reason": reason,
                "improvements": improvements,
        ...
        summary.setdefault("validation_history", []).append(
            jsonable_encoder(validation_after or {})
        )
        ...
        if candidate_after or candidate_snapshot:
            selection = (
                (candidate_after or candidate_snapshot or {}).get("detection", {}).get("selection")
                if isinstance(candidate_after or candidate_snapshot, dict)
                else None
            )
            summary.setdefault("candidate_selections", []).append(jsonable_encoder(selection or {}))
        if trust_score is not None:
            summary.setdefault("trust_progression", []).append(
                {
                    "graph_run_id": graph_run_id,
                    "score": trust_score,
                    "recommendation": recommendation,
    ...
    def attach_proposal_revision(
        self, graph_run_id: str, proposal_id: str, proposal_revision_id: str
    ) -> None:
        session = self.session_for_run(graph_run_id)
        if not session:
            return
    ...
    def finish_session(
        self, graph_run_id: str, status: str, termination_reason: str | None
    ) -> None:
        session = self.session_for_run(graph_run_id)
        if session:
    ...
    def _improvements(
        self, before: dict[str, Any] | None, after: dict[str, Any] | None, confidence_delta: float
    ) -> list[str]:
        ...
        if before and after and before.get("valid") != after.get("valid"):
            improvements.append(
                f"validation changed from {before.get('valid')} to {after.get('valid')}"
            )
    ...
    def _memory(self, session: AiReasoningSession) -> dict[str, Any]:
        summary = dict(session.session_summary or {})
        memory_defaults: dict[str, list[Any]] = {
            "accepted_attack": [],
            "rejected_attack": [],
            "watcher_failures": [],
            "validation_history": [],
...
class ConsumptionStatusService:
    ...
    def summary(self) -> dict[str, Any]:
        ...
        if scheduled_next:
            ...
            next_poll = (
                last_success + timedelta(seconds=settings.misp_poll_interval_seconds)
            ).isoformat()
        else:
            next_poll = None
        counts = {
            status: self.db.scalar(
                select(func.count())
                .select_from(CtiConsumptionRecord)
                .where(CtiConsumptionRecord.status == status)
            )
            or 0
            for status in [
                "pending",
        ...
        records = self.db.scalars(
            select(CtiConsumptionRecord).order_by(CtiConsumptionRecord.created_at.desc()).limit(25)
        ).all()
...
def latest_node_duration(db: Session, graph_run_id: str, node_name: str) -> int | None:
    node = db.scalars(
        select(GraphNodeRun)
        .where(GraphNodeRun.graph_run_id == graph_run_id, GraphNodeRun.node_name == node_name)
