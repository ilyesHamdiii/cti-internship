...
class AiProvider(ABC):
    ...
    @abstractmethod
    async def analyze_cti(self, normalized_cti: dict[str, Any]) -> tuple[CtiAnalysisResponse, dict[str, Any]]:
        raise NotImplementedError

    @abstractmethod
    async def generate_sigma(self, payload: dict[str, Any]) -> tuple[SigmaGenerationResponse, dict[str, Any]]:
        raise NotImplementedError

    @abstractmethod
    async def repair_sigma(self, payload: dict[str, Any]) -> tuple[SigmaRepairResponse, dict[str, Any]]:
        raise NotImplementedError
...
class DeterministicFixtureProvider(AiProvider):
    ...
    async def analyze_cti(self, normalized_cti: dict[str, Any]) -> tuple[CtiAnalysisResponse, dict[str, Any]]:
        ...
        if "wmic" in lowered or "wmi" in lowered:
            behaviors.append(behavior("wmi", "WMI process execution", "process_execution", "execute", "remote process", "wmic.exe", "T1047", "Windows Management Instrumentation", "execution", "Execution", "process", ["process_name", "command_line"]))
        if "schtasks" in lowered or "scheduled task" in lowered:
            behaviors.append(behavior("scheduled_task", "Scheduled task persistence", "persistence", "persist", "scheduled task", "schtasks.exe", "T1053.005", "Scheduled Task", "persistence", "Persistence", "process", ["process_name", "command_line"]))
        if "credential" in lowered or "lsass" in lowered or "mimikatz" in lowered:
            behaviors.append(behavior("credential_dumping", "Credential dumping from LSASS", "credential_access", "dump", "lsass memory", "process access", "T1003.001", "LSASS Memory", "credential-access", "Credential Access", "process", ["process_name", "command_line"]))
        if "rundll32" in lowered:
            behaviors.append(behavior("rundll32", "Suspicious rundll32 execution", "process_execution", "execute", "DLL entry point", "rundll32.exe", "T1218.011", "Rundll32", "defense-evasion", "Defense Evasion", "process", ["process_name", "command_line"]))
        if "powershell" in lowered or "-enc" in lowered or "encodedcommand" in lowered:
            behaviors.append(behavior("powershell_encoded", "PowerShell encoded command execution", "process_execution", "execute", "PowerShell", "powershell.exe -EncodedCommand", "T1059.001", "PowerShell", "execution", "Execution", "process", ["process_name", "command_line"]))
        if "missing telemetry" in lowered or "cloud-only" in lowered:
            behaviors.append(behavior("cloud_visibility_gap", "Cloud control-plane action without telemetry", "cloud_activity", "modify", "cloud control plane", "cloud api", "T1098", "Account Manipulation", "persistence", "Persistence", "cloud_audit", ["event_name", "user", "source_ip"]))
        if not behaviors:
            behaviors.append(behavior("low_confidence", "Insufficiently specific suspicious activity", "unknown", "unknown", "unknown", "unknown", "T1059.001", "PowerShell", "execution", "Execution", "process", ["process_name"], confidence=0.30))
        if "supervisor_invalid_sigma" in lowered:
            behaviors[0]["summary"] = f"{behaviors[0]['summary']} supervisor_invalid_sigma"
            behaviors[0]["observables"].append({"type": "test_marker", "value": "supervisor_invalid_sigma"})
        if "supervisor_safety_fail" in lowered:
            behaviors[0]["summary"] = f"{behaviors[0]['summary']} supervisor_safety_fail"
            behaviors[0]["observables"].append({"type": "test_marker", "value": "supervisor_safety_fail"})
        payload = {
            "behaviors": behaviors,
        ...
        return response, usage("cti_behavior_extraction_v2", normalized_cti, response.model_dump(), "fixture", "deterministic-fixture")

    async def generate_sigma(self, payload: dict[str, Any]) -> tuple[SigmaGenerationResponse, dict[str, Any]]:
        response = SigmaGenerationResponse.model_validate(build_sigma_response(payload, repaired=False))
        return response, usage("sigma_generation_v2", payload, response.model_dump(), "fixture", "deterministic-fixture")
    ...
    async def repair_sigma(self, payload: dict[str, Any]) -> tuple[SigmaRepairResponse, dict[str, Any]]:
        ...
        return response, usage("sigma_repair_v2", payload, response.model_dump(), "fixture", "deterministic-fixture")
...
class DeepSeekLiveProvider(AiProvider):
    ...
    async def _call(self, prompt_version: str, payload: dict[str, Any], schema: type[T]) -> tuple[T, dict[str, Any]]:
        ...
        for attempt in range(3):
            try:
                async with httpx.AsyncClient(timeout=30) as client:
                    result = await client.post(
                        ...
                        json={
                            ...
                            "messages": [
                                ...
                                {"role": "user", "content": json.dumps({"prompt_version": prompt_version, "schema": schema.model_json_schema(), "input": payload})},
                            ],
                    ...
                    meta = usage(prompt_version, payload, parsed.model_dump(), "deepseek", self.settings.deepseek_model, data.get("usage", {}))
        ...
        raise DeepSeekProviderError(sanitize_provider_error(last_error, prompt_version, time.perf_counter() - started))
...
class DeepSeekClient:
    def __init__(self, settings: Settings | None = None):
        ...
        self.provider: AiProvider = DeterministicFixtureProvider() if self.settings.ai_fixture_mode else DeepSeekLiveProvider(self.settings)
...
def evidence_text(normalized_cti: dict[str, Any]) -> str:
    ...
    return "\n".join([str(normalized_cti.get("title", "")), str(normalized_cti.get("evidence_text", "")), *values])


def behavior(key: str, summary: str, behavior_type: str, action: str, target: str, mechanism: str, technique_id: str, technique_name: str, tactic_id: str, tactic_name: str, telemetry_category: str, fields: list[str], confidence: float = 0.88) -> dict[str, Any]:
...
def build_sigma_response(payload: dict[str, Any], repaired: bool) -> dict[str, Any]:
    behavior_payload = payload.get("behavior") or payload.get("behavior_summary") or payload.get("candidate", {}).get("title", "")
    ...
    if "wmi" in text:
        title, selection, tags = "WMI Process Execution", {"Image|endswith": "\\wmic.exe"}, ["attack.T1047"]
    elif "scheduled" in text or "schtasks" in text:
        title, selection, tags = "Scheduled Task Persistence", {"Image|endswith": "\\schtasks.exe"}, ["attack.T1053.005"]
    elif "credential" in text or "lsass" in text:
        title, selection, tags = "Potential LSASS Credential Dumping", {"CommandLine|contains": "lsass"}, ["attack.T1003.001"]
    elif "rundll32" in text:
        title, selection, tags = "Suspicious Rundll32 Execution", {"Image|endswith": "\\rundll32.exe"}, ["attack.T1218.011"]
    else:
        title, selection, tags = "PowerShell Encoded Command Execution", {"Image|endswith": "\\powershell.exe", "CommandLine|contains": "-enc"}, ["attack.T1059.001"]
    response = {
        "sigma": {
        ...
        "structured_justification": justification(["repaired" if repaired else "generated_from_behavior"], [], confidence=0.86, changes=["Applied analyst instruction"] if repaired and analyst_comment else []),
        "confidence": 0.86,
        "uncertainties": [],
    }
...
def plan_semantic_repair(payload: dict[str, Any], sigma: dict[str, Any]) -> dict[str, Any]:
    ...
    context = json.dumps({"behavior": behavior, "candidate": candidate}, sort_keys=True, default=str).lower()
    quoted_literals = [match.strip() for match in re.findall(r"[\"']([^\"']{1,120})[\"']", analyst_comment) if match.strip()]
    lowered = analyst_comment.lower()

    added_values: list[str] = []
    evidence: list[dict[str, object]] = []
    interpretation = "No actionable Sigma selection change was inferred from the analyst instruction."
    unresolved: list[str] = []
    uncertainty: list[str] = []

    if any(term in lowered for term in ["download", "remote content", "webrequest", "web request", "webclient", "bits", "curl", "wget"]):
        if "powershell" in context or "commandline" in json.dumps(candidate, sort_keys=True, default=str).lower():
            added_values.extend(REMOTE_DOWNLOAD_PATTERNS)
            interpretation = "Require evidence of remote content download behavior in the process command line."
            evidence.append(
                {
                    "field": "CommandLine",
            ...
            uncertainty.append("The source behavior does not expose compatible process command-line telemetry.")
    elif any(str(error.get("code")) == "missing_condition" for error in validation.get("errors", []) if isinstance(error, dict)) or "supervisor_invalid_sigma" in context:
        interpretation = "Restore the Sigma condition field required by deterministic validation."

    for literal in quoted_literals:
        if literal.lower() not in {word.lower() for word in ORDINARY_INSTRUCTION_WORDS} and literal not in added_values:
            added_values.append(literal)
            interpretation = "Apply explicit analyst-provided technical literal to command-line detection."
            evidence.append(
                {
                    "field": "CommandLine",
                    "values": [literal],
    ...
    sanitized_values = [value for value in added_values if value.strip().lower() not in ORDINARY_INSTRUCTION_WORDS]
    ...
    elif sanitized_values and any(item.get("source") == "recognized_behavioral_pattern" for item in evidence):
        change_summary = [f"Added CommandLine remote-content download patterns: {', '.join(sanitized_values)}"]
    elif sanitized_values:
        change_summary = [f"Added CommandLine analyst-provided technical literals: {', '.join(sanitized_values)}"]
    else:
        change_summary = ["No semantic Sigma selection change applied"]

    return {
        "instruction_interpretation": interpretation,
        "fields_changed": ["detection.condition"] if missing_condition_repair else ["CommandLine"] if sanitized_values else [],
...
def apply_repair_plan(sigma: dict[str, Any], plan: dict[str, Any]) -> dict[str, Any]:
    repaired = json.loads(json.dumps(sigma))
    ...
    if current_values and all(str(value).startswith("-") for value in current_values):
        ...
        selection["CommandLine|contains|all"] = list(dict.fromkeys([*map(str, selection.get("CommandLine|contains|all", [])), *map(str, current_values)]))
        current_values = []
    selection["CommandLine|contains"] = list(dict.fromkeys([*map(str, current_values), *map(str, added_values)]))
    return repaired


def justification(factors: list[str], evidence_refs: list[dict[str, Any]], confidence: float, changes: list[str] | None = None) -> dict[str, Any]:
    return {
...
def usage(prompt_version: str, request: dict[str, Any], response: dict[str, Any], provider: str, model: str, token_usage: dict[str, int] | None = None) -> dict[str, Any]:
    token_usage = token_usage or {"prompt_tokens": 100, "completion_tokens": 100, "total_tokens": 200}
    ...
    return {
        ...
        "estimated_cost": token_usage.get("prompt_tokens", 0) * settings.deepseek_prompt_token_cost + token_usage.get("completion_tokens", 0) * settings.deepseek_completion_token_cost,
        "latency_ms": 0,
    }


def sanitize_provider_error(error: Exception | None, prompt_version: str, elapsed_seconds: float) -> dict[str, Any]:
    status_code = None
