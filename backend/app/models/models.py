...
class CtiEvent(Base):
    ...
    status: Mapped[CtiEventStatus] = mapped_column(Enum(CtiEventStatus), default=CtiEventStatus.pending, nullable=False)
...
class Workflow(Base):
    ...
    id: Mapped[str] = mapped_column(UUID(as_uuid=False), primary_key=True, default=lambda: str(uuid4()))
    cti_event_id: Mapped[str] = mapped_column(ForeignKey("cti_events.id"), unique=True, nullable=False)
...
class GraphRun(Base):
    ...
    status: Mapped[GraphRunStatus] = mapped_column(Enum(GraphRunStatus), default=GraphRunStatus.pending, nullable=False)
...
class AiInteraction(Base):
    ...
    __table_args__ = (UniqueConstraint("idempotency_key", name="uq_ai_interaction_idempotency_key"),)
...
class AiReasoningSession(Base):
    ...
    id: Mapped[str] = mapped_column(UUID(as_uuid=False), primary_key=True, default=lambda: str(uuid4()))
    workflow_id: Mapped[str] = mapped_column(ForeignKey("workflows.id"), nullable=False)
    graph_run_id: Mapped[str] = mapped_column(ForeignKey("graph_runs.id"), unique=True, nullable=False)
    ...
    approval_recommendation: Mapped[str] = mapped_column(String(50), default="needs_review", nullable=False)
...
class AiReasoningRevision(Base):
    ...
    graph_run_id: Mapped[str] = mapped_column(ForeignKey("graph_runs.id"), nullable=False)
    parent_reasoning_revision_id: Mapped[str | None] = mapped_column(ForeignKey("ai_reasoning_revisions.id"))
    ...
    watcher_results: Mapped[list[dict[str, Any]]] = mapped_column(JSONB, default=list, nullable=False)
...
class DetectionCatalog(Base):
    ...
    status: Mapped[DetectionStatus] = mapped_column(Enum(DetectionStatus), default=DetectionStatus.active, nullable=False)
    created_at: Mapped[datetime] = mapped_column(DateTime, default=now, nullable=False)
    updated_at: Mapped[datetime] = mapped_column(DateTime, default=now, onupdate=now, nullable=False)
...
class DetectionTelemetryRequirement(Base):
    ...
    id: Mapped[str] = mapped_column(UUID(as_uuid=False), primary_key=True, default=lambda: str(uuid4()))
    detection_id: Mapped[str] = mapped_column(ForeignKey("detection_catalog.id"), nullable=False)
    telemetry_source_id: Mapped[str] = mapped_column(ForeignKey("telemetry_sources.id"), nullable=False)
...
class VisibilityResult(Base):
    ...
    graph_run_id: Mapped[str] = mapped_column(ForeignKey("graph_runs.id"), nullable=False)
    behavior_id: Mapped[str] = mapped_column(ForeignKey("behaviors.id"), nullable=False)
    visibility_status: Mapped[VisibilityStatus] = mapped_column(Enum(VisibilityStatus), nullable=False)
...
class Proposal(Base):
    ...
    id: Mapped[str] = mapped_column(UUID(as_uuid=False), primary_key=True, default=lambda: str(uuid4()))
    behavior_id: Mapped[str] = mapped_column(ForeignKey("behaviors.id"), unique=True, nullable=False)
    workflow_id: Mapped[str] = mapped_column(ForeignKey("workflows.id"), nullable=False)
    status: Mapped[ProposalStatus] = mapped_column(Enum(ProposalStatus), default=ProposalStatus.ready_review, nullable=False)
...
class ProposalRevision(Base):
    ...
    __table_args__ = (UniqueConstraint("proposal_id", "revision_number", name="uq_revision_number"),)
...
class ValidationResult(Base):
    ...
    id: Mapped[str] = mapped_column(UUID(as_uuid=False), primary_key=True, default=lambda: str(uuid4()))
    proposal_revision_id: Mapped[str] = mapped_column(ForeignKey("proposal_revisions.id"), nullable=False)
...
class ReviewAction(Base):
    ...
    __table_args__ = (UniqueConstraint("proposal_revision_id", "action", name="uq_review_action_revision_action"),)
...
class DeploymentArtifact(Base):
    ...
    __table_args__ = (UniqueConstraint("proposal_revision_id", "artifact_type", name="uq_deployment_artifact_revision_type"),)
